 ...
 * 0            4            8            12           16
 * +------------+------------+------------+------------+
 * | 0xT0T1T2T3 | 0xT4T5T6T7 | 0xT8T9TATB | 0xTCTDTETF |
 * +------------+------------+------------+------------+
 *      T[0]         T[1]         T[2]         T[3]
 *
...
typedef blkcphr_u32_t cast256_word_t;

typedef cast256_word_t cast256_block_t[4];
typedef cast256_word_t cast256_chunk_t[8];

static const cast256_word_t cast256i_S1[256];
static const cast256_word_t cast256i_S2[256];
static const cast256_word_t cast256i_S3[256];
static const cast256_word_t cast256i_S4[256];

static const cast256_chunk_t cast256i_Tm[24];
static const cast256_chunk_t cast256i_Tr[24];
...
static cast256_word_t cast256i_F1(cast256_word_t in, cast256_word_t km, cast256_word_t kr) {
    cast256_word_t out;
    in = cast256i_rotl(km + in, kr);
    out  = cast256i_S1[LL(in)];
    out ^= cast256i_S2[LR(in)];
    out -= cast256i_S3[RL(in)];
    out += cast256i_S4[RR(in)];
    return out;
}

static cast256_word_t cast256i_F2(cast256_word_t in, cast256_word_t km, cast256_word_t kr) {
    cast256_word_t out;
    in = cast256i_rotl(km ^ in, kr);
    out  = cast256i_S1[LL(in)];
    out -= cast256i_S2[LR(in)];
...
static cast256_word_t cast256i_F3(cast256_word_t in, cast256_word_t km, cast256_word_t kr) {
    cast256_word_t out;
    in = cast256i_rotl(km - in, kr);
    out  = cast256i_S1[LL(in)];
    out += cast256i_S2[LR(in)];
...
static void cast256i_write_to_chunk(cast256_chunk_t out, const void* src, int words) {
    ...
    BLKCPHR_IF(BLKCPHR_IS_LITTLE, BLKCPHR_BSWAP_32x8(out));
...
static void cast256i_kappa(cast256_chunk_t x, int i) {
    x[6] ^= cast256i_F1(x[7], cast256i_Tm[i][0], cast256i_Tr[i][0]);
    x[5] ^= cast256i_F2(x[6], cast256i_Tm[i][1], cast256i_Tr[i][1]);
    x[4] ^= cast256i_F3(x[5], cast256i_Tm[i][2], cast256i_Tr[i][2]);
    x[3] ^= cast256i_F1(x[4], cast256i_Tm[i][3], cast256i_Tr[i][3]);
    x[2] ^= cast256i_F2(x[3], cast256i_Tm[i][4], cast256i_Tr[i][4]);
    x[1] ^= cast256i_F3(x[2], cast256i_Tm[i][5], cast256i_Tr[i][5]);
    x[0] ^= cast256i_F1(x[1], cast256i_Tm[i][6], cast256i_Tr[i][6]);
    x[7] ^= cast256i_F2(x[0], cast256i_Tm[i][7], cast256i_Tr[i][7]);
}

int cast256_init(const void* key, int bits) {
    cast256_chunk_t X = {0}; int i;

    if (bits < 128 || bits > 256) return 1;
    if (bits % 32 != 0) return 1;

    cast256i_write_to_chunk(X, key, bits / 32);

    for (i = 0; i < 12; i++) {
        cast256i_kappa(X, 2 * i + 0);
        cast256i_kappa(X, 2 * i + 1);
        cast256i_ctx.Km[i][0] = X[7]; cast256i_ctx.Km[i][1] = X[5];
        cast256i_ctx.Km[i][2] = X[3]; cast256i_ctx.Km[i][3] = X[1];
        cast256i_ctx.Kr[i][0] = X[0]; cast256i_ctx.Kr[i][1] = X[2];
        cast256i_ctx.Kr[i][2] = X[4]; cast256i_ctx.Kr[i][3] = X[6];
    }
...
static void cast256i_write_to_block(cast256_block_t out, const void* src) {
    ...
    BLKCPHR_IF(BLKCPHR_IS_LITTLE, BLKCPHR_BSWAP_32x4(out));
/* #if BLKCPHR_IS_LITTLE
    out[0] = blkcphr_bswap32(out[0]);
    out[1] = blkcphr_bswap32(out[1]);
    out[2] = blkcphr_bswap32(out[2]);
...
static void cast256i_beta_fwd(cast256_block_t x, int i) {
    x[2] ^= cast256i_F1(x[3], cast256i_ctx.Km[i][0], cast256i_ctx.Kr[i][0]);
    x[1] ^= cast256i_F2(x[2], cast256i_ctx.Km[i][1], cast256i_ctx.Kr[i][1]);
    x[0] ^= cast256i_F3(x[1], cast256i_ctx.Km[i][2], cast256i_ctx.Kr[i][2]);
    x[3] ^= cast256i_F1(x[0], cast256i_ctx.Km[i][3], cast256i_ctx.Kr[i][3]);
}

static void cast256i_beta_inv(cast256_block_t x, int i) {
    x[3] ^= cast256i_F1(x[0], cast256i_ctx.Km[i][3], cast256i_ctx.Kr[i][3]);
    x[0] ^= cast256i_F3(x[1], cast256i_ctx.Km[i][2], cast256i_ctx.Kr[i][2]);
    x[1] ^= cast256i_F2(x[2], cast256i_ctx.Km[i][1], cast256i_ctx.Kr[i][1]);
    x[2] ^= cast256i_F1(x[3], cast256i_ctx.Km[i][0], cast256i_ctx.Kr[i][0]);
...
void cast256_block_encode(void* dst, const void* src) {
    cast256_block_t X; int i;
    cast256i_write_to_block(X, src);

    for (i = 0; i <  6; i++) cast256i_beta_fwd(X, i);
    for (i = 6; i < 12; i++) cast256i_beta_inv(X, i);

    cast256i_read_from_block(dst, X);
}

void cast256_block_decode(void* dst, const void* src) {
    cast256_block_t X; int i;
    cast256i_write_to_block(X, src);

    for (i = 0; i <  6; i++) cast256i_beta_fwd(X, 11 - i);
    for (i = 6; i < 12; i++) cast256i_beta_inv(X, 11 - i);
...
static const cast256_chunk_t cast256i_Tr[24] = {
    { 0x00000013, 0x00000004, 0x00000015, 0x00000006, 0x00000017, 0x00000008, 0x00000019, 0x0000000a },
    { 0x0000001b, 0x0000000c, 0x0000001d, 0x0000000e, 0x0000001f, 0x00000010, 0x00000001, 0x00000012 },
    { 0x00000003, 0x00000014, 0x00000005, 0x00000016, 0x00000007, 0x00000018, 0x00000009, 0x0000001a },
    { 0x0000000b, 0x0000001c, 0x0000000d, 0x0000001e, 0x0000000f, 0x00000000, 0x00000011, 0x00000002 },
    { 0x00000013, 0x00000004, 0x00000015, 0x00000006, 0x00000017, 0x00000008, 0x00000019, 0x0000000a },
    { 0x0000001b, 0x0000000c, 0x0000001d, 0x0000000e, 0x0000001f, 0x00000010, 0x00000001, 0x00000012 },
    { 0x00000003, 0x00000014, 0x00000005, 0x00000016, 0x00000007, 0x00000018, 0x00000009, 0x0000001a },
    { 0x0000000b, 0x0000001c, 0x0000000d, 0x0000001e, 0x0000000f, 0x00000000, 0x00000011, 0x00000002 },
